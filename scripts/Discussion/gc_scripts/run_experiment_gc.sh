...
if [ ! -f "projects_evaluated_gc.csv" ]; then
    echo "❌ Error: projects_evaluated_gc.csv not found!"
    exit 1
fi

# Count total projects (minus header)
total_projects=$(( $(wc -l < projects_evaluated_gc.csv) - 1 ))
current_index=0

# Read the first line but don't process it
read -r first_line < projects_evaluated_gc.csv 
echo "Skipping first line: $first_line"

scripts=("run_pymop_no_gc.sh" "run_pymop_gc.sh")

# Process the remaining lines
sed 1d projects_evaluated_gc.csv | while IFS=, read -r project repo_url target_sha; do
