...
def main():
    ...
    for project in sorted(projects):
        ...
        pymop_gc_folder = f"{project}_Pymop_GC/{project}_Pymop_GC"
        pymop_no_gc_folder = f"{project}_Pymop_NO_GC/{project}_Pymop_NO_GC"
        ...
        # ===============================================
        # Process Pymop files with GC
        # ===============================================

        # If no pymop folder, print error and skip to next project
        if not os.path.exists(pymop_gc_folder):
            print(f'No pymop with GC folder found for {project}')
            line = OrderedDict({
                'project': project,
                'algorithm': 'D',
                'passed': 'x',
            ...
            for algorithm in ["D"]:
                ...
                # If no result or output files, print error and skip to next project
                if not result_files or not output_files:
                    print(f'No pymop with GC files found for {project}')
                    result_file = None
                    output_file = None
                else:
                    result_file = result_files[0]
        ...
        # ===============================================
        # Process Pymop files with NO GC
        # ===============================================

        # If no pymop folder, print error and skip to next project
        if not os.path.exists(pymop_no_gc_folder):
            print(f'No pymop with NO GC folder found for {project}')
            line = OrderedDict({
                'project': project,
                'algorithm': 'D',
                'passed': 'x',
            ...
            for algorithm in ["D"]:
                ...
                # If no result or output files, print error and skip to next project
                if not result_files or not output_files:
                    print(f'No pymop with NO GC files found for {project}')
                    result_file = None
                    output_file = None
                else:
                    result_file = result_files[0]
