...
PYTHONPATH="$PWD"/../pymop-artifacts-rv/pymop/pythonmop/pymop-startup-helper timeout -k 9 3000 pytest --path="$PWD"/../pymop-artifacts-rv/pymop/specs-new --instrument_strategy=ast --algo=D --memray --trace-python-allocators --most-allocations=0 --memray-bin-path=./$MEMORY_DATA_DIR_NAME --continue-on-collection-errors --json-report --json-report-indent=2 --statistics --statistics_file="D".json > ${TESTING_REPO_NAME}_Output.txt
exit_code=$?
